...
function register({ config: { rows, cols, autoPlay, ...unknownOptions } }) {
  ...
  this.on('uiLoaded', async ({ playbook, uiCatalog }) => {
    ...
    if (!uiCatalog.findByType('partial').some(({ path }) => path === asciinemaLoadScriptsPartialPath)) {
      const asciinemaLoadScriptsPartialFilepath = ospath.join(__dirname, asciinemaLoadScriptsPartialPath)
      uiCatalog.addFile({
    ...
    if (!uiCatalog.findByType('partial').some(({ path }) => path === asciinemaCreateScriptsPartialPath)) {
      const asciinemaCreateScriptsPartialFilepath = ospath.join(__dirname, asciinemaCreateScriptsPartialPath)
      uiCatalog.addFile({
    ...
    const splitHelperPartialFilepath = ospath.join(__dirname, splitHelperPartialPath)
    uiCatalog.addFile({
...
const toBlock = (attrs, parent, source, context, uiCatalog, defaultOptions, file) => {
    ...
    if (file.asciidoc.attributes['page-asciinemacasts']) {
      file.asciidoc.attributes['page-asciinemacasts'] = file.asciidoc.attributes['page-asciinemacasts'] + ',' + asciinemaId
    }
    else {
