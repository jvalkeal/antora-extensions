...
function register({ config: { rows, cols, autoPlay, ...unknownOptions } }) {
  ...
  this.on('uiLoaded', async ({ playbook, uiCatalog }) => {
    ...
    const asciinemaScriptsPartialPath = 'asciinema-load.hbs'
    if (!uiCatalog.findByType('partial').some(({ path }) => path === asciinemaScriptsPartialPath)) {
      const asciinemaScriptsPartialFilepath = ospath.join(__dirname, asciinemaScriptsPartialPath)
      uiCatalog.addFile({
    ...
    const asciinemaFooterScriptsPartialPath = 'asciinema-create.hbs'
    if (!uiCatalog.findByType('partial').some(({ path }) => path === asciinemaFooterScriptsPartialPath)) {
      const asciinemaFooterScriptsPartialFilepath = ospath.join(__dirname, asciinemaFooterScriptsPartialPath)
      uiCatalog.addFile({
        contents: Buffer.from(template(await fsp.readFile(asciinemaFooterScriptsPartialFilepath, 'utf8'), {  })),
        path: asciinemaFooterScriptsPartialPath,
        stem: 'asciinema-footer-scripts',
        type: 'partial',
      })
    }
    ...
    if (!uiCatalog.findByType('partial').some(({ path }) => path === asciinemaStylesPartialPath)) {
      const asciinemaStylesPartialFilepath = ospath.join(__dirname, asciinemaStylesPartialPath)
      uiCatalog.addFile({
    ...
    const helperPartialPath = 'asciinema-split-helper.js'
    const helperPartialFilepath = ospath.join(__dirname, helperPartialPath)
    uiCatalog.addFile({
      contents: Buffer.from(template(await fsp.readFile(helperPartialFilepath, 'utf8'), {  })),
      path: 'helpers/' + helperPartialPath,
      stem: 'asciinema-split',
      type: 'helper'
    })

    const optionsHelperPartialPath = 'asciinema-options-helper.js'
    const optionsHelperPartialFilepath = ospath.join(__dirname, optionsHelperPartialPath)
    uiCatalog.addFile({
...
const toBlock = (attrs, parent, source, context, uiCatalog, defaultOptions, file) => {
    ...
    if (file.asciidoc.attributes['page-foobar1']) {
      file.asciidoc.attributes['page-foobar1'] = file.asciidoc.attributes['page-foobar1'] + ',' + asciinemaId
    }
    else {
      file.asciidoc.attributes['page-foobar1'] = asciinemaId
    }
